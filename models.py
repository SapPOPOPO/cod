...
class SASRecModel(nn.Module):
    ...
    def transformer_encoder_from_embeds(self, inputs_embeds, input_ids):
        """
        Accept pre-computed embeddings instead of item IDs.
        """
        attention_mask          = (input_ids > 0).long()
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)

        # Causal mask — identical to transformer_encoder
        max_len         = attention_mask.size(-1)
        attn_shape      = (1, max_len, max_len)
        ...
        # Position embeddings added on top of pre-computed embeddings
        seq_length   = inputs_embeds.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long,
                                    device=inputs_embeds.device).unsqueeze(0)
        pos_emb      = self.position_embeddings(position_ids)
        sequence_emb = self.LayerNorm(self.dropout(inputs_embeds + pos_emb))
